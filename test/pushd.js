...
// For testing paths, normalize to unix
var tpath = {
    relative: function(from, to)
    {
        var p = path.relative(from, to);
        if(path.sep !== '/')
            p = p.replace(tpath._reg, '/');
        return p;
    }
};
tpath._reg = new RegExp(path.sep.replace('\\', '\\\\'), 'g');
...
assert.equal(tpath.relative(root, trail[0]), 'resources/pushd');
...
assert.equal(tpath.relative(root, trail[1]), 'resources/pushd');
assert.equal(process.cwd(), trail[0]);
...
assert.equal(tpath.relative(root, trail[0]), 'resources/pushd/a');
assert.equal(trail[1], root);
assert.equal(tpath.relative(root, trail[2]), 'resources/pushd');
assert.equal(process.cwd(), trail[0]);
...
assert.equal(trail.length, 3);
assert.equal(trail[0], root);
assert.equal(tpath.relative(root, trail[1]), 'resources/pushd/a');
assert.equal(tpath.relative(root, trail[2]), 'resources/pushd');
assert.equal(process.cwd(), trail[0]);
